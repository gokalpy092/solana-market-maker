...
try:
    ...
    market = mango.PerpMarket.ensure(mango.market(context, MARKET))
    ...
    ratios_element = RatiosElement(
        mango.OrderType.POST_ONLY_SLIDE, 30, 30, [Decimal("0.001")], [Decimal("0.05")], False
    )
    ...
    #model_state_builder = mango.marketmaking.modelstatebuilderfactory._polling_perp_model_state_builder_factory(
       # group, account, market, oracle)
    model_state_builder = mango.marketmaking.PerpPollingModelStateBuilder(
        account.address, market, oracle, group.address, group.cache, account.spot_open_orders
    )
